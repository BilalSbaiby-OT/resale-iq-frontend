...
def main():
    ...
    if not args.no_run:
        ...
        # Only write when the content actually changed. The hourly job would
        # otherwise touch a tracked file every hour, leaving the tree permanently
        # dirty and the Stop gate permanently blocked — an automation that makes
        # the Definition of Done unsatisfiable is worse than no automation.
        body = ("<!-- GENERATED by scripts/company/status_report.py. Do not edit:\n"
                "     it is regenerated from check output, and a hand-written status\n"
                "     is exactly the drift this file exists to prevent. -->\n\n```\n"
                + render(data, prev) + "\n```\n")
        try:
            if open(STATUS_MD).read() == body:
                print(text)
