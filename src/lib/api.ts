...
// This is the only data in the product that is not generated by us, which is
// what makes it the only honest test of whether the advice was any good.
export type PendingOutcome = {
  id: number
  query: string
  verdict: string
  said_buy_below: number | null
  said_sell_avg: number | null
  created_at: string
}
export const getPendingOutcomes = (limit = 3) =>
  request<{ pending: PendingOutcome[]; count: number }>(`/api/outcomes/pending?limit=${limit}`)

export const reportOutcome = (body: {
  verdict_log_id: number
  did_buy: boolean
  buy_price_eur?: number | null
  did_sell?: boolean | null
  sell_price_eur?: number | null
  days_to_sell?: number | null
  note?: string
}) =>
  request<{ success: boolean; result: string }>("/api/outcomes", {
    method: "POST",
