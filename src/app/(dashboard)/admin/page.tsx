...
function AdminInner() {
  ...
  return (
    <>
      ...
      <div style={{ background: "var(--color-surface)", border: "1px solid var(--color-border-ui)", borderRadius: 10, overflow: "hidden" }}>
        <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--color-border-ui)", display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search email"
            style={{ background: "var(--color-surface-elevated)", border: "1px solid var(--color-border-2)", borderRadius: 7, color: "#eef1f7", fontSize: 13, padding: "7px 12px", minWidth: 220, flex: "1 1 180px" }} />
        ...
        {msg && <div style={{ padding: "8px 16px", fontSize: 12, color: "#34d399", borderBottom: "1px solid var(--color-border-ui)" }}>{msg}</div>}
                  ...
                  <td style={TD}>
                    {u.is_owner ? (
                      ...
                      <select value={u.plan} onChange={e => changePlan(u, e.target.value)} disabled={busy === u.id}
                        style={{ background: "var(--color-surface-elevated)", border: "1px solid var(--color-border-2)", borderRadius: 6, color: PLAN_COLOR[u.plan] || "#eef1f7", fontSize: 12, padding: "5px 8px", fontWeight: 600 }}>
                  ...
                  <td style={{ ...TD, whiteSpace: "nowrap", textAlign: "right" }}>
                    {!u.is_owner && (
                      <button onClick={() => setActive(u, !on)} disabled={busy === u.id}
                        style={{ fontSize: 12, padding: "5px 10px", borderRadius: 6, cursor: "pointer", background: "transparent", border: "1px solid var(--color-border-2)", color: on ? "#f87171" : "#34d399", marginRight: 6 }}>
                        {on ? "Disable" : "Enable"}
                      </button>
                    )}
