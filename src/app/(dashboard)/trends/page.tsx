                ...
                (data?.trending_models ?? []).filter(m => m.momentum_label === "HOT" || m.momentum_label === "RISING").slice(0, 10).map((m, i) => (
                  <div key={i} className="flex items-center gap-2.5 px-4 py-2.5 hover:bg-[var(--color-surface-elevated)]">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[12px] text-[var(--color-text-secondary)]">{m.brand} {m.momentum_label && <MomentumBadge momentum={m.momentum_label} sold7={m.sold_7d} sold30={m.sold_30d} />}</div>
              ...
              {loading ? <div className="text-center py-8 text-[var(--color-text-secondary)] text-[12px]">Loading…</div> :
                [...(data?.trending_models ?? [])]
                  ...
                  <div key={i} className="flex items-center gap-2.5 px-4 py-2.5 hover:bg-[var(--color-surface-elevated)]">
                    <span className="font-mono text-[12px] text-[var(--color-text-secondary)] w-4">{i + 1}</span>
                    <div className="flex-1 min-w-0">
                      <div className="text-[12px] text-[var(--color-text-secondary)]">{m.brand}</div>
                      <div className="font-semibold text-[13px] truncate">{m.model} <span className="text-[var(--color-text-secondary)]">· {m.category}</span></div>
                    </div>
                    <ScoreBar score={m.opportunity_score} />
                  </div>
                ))
