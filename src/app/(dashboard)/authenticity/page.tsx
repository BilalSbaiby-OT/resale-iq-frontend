        ...
        <div className="bg-[#141820] border border-[#1e2535] rounded-xl p-5 mb-4">
          ...
          <div className="flex gap-2">
            <input value={url} onChange={e => setUrl(e.target.value)} onKeyDown={e => e.key === "Enter" && analyzeUrl()}
              placeholder="https://www.vinted.es/items/..." className="flex-1 bg-[#1a2030] border border-[#263147] rounded-lg px-3 py-2.5 font-mono text-[13px] text-[#e8ecf4] outline-none focus:border-emerald-500/60 placeholder:text-[#546380]" />
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            {[["Listed price (€)*","listed_price","45","number"],["Brand","brand","Gucci","text"],["Model","model","GG Belt","text"],["Category","category","Bags","text"],["Seller account age (days)","seller_days","365","number"],["Seller rating (0-5)","seller_rating","4.8","number"],["Seller total sales","seller_sales","42","number"]].map(([l,k,ph,t]) => (
              <div key={String(k)} className={k === "listed_price" ? "col-span-2" : ""}>
                ...
                <input type={String(t)} value={form[k as keyof typeof form]} onChange={e => set(String(k), e.target.value)} placeholder={String(ph)} className="w-full bg-[#1a2030] border border-[#263147] rounded-lg px-3 py-2 text-[13px] text-[#e8ecf4] outline-none focus:border-emerald-500/60 placeholder:text-[#546380]" />
              </div>
            ))}
            <div className="col-span-2">
              <label className="text-[10px] text-[#546380] tracking-wide block mb-1.5">Listing title (optional)</label>
              <input value={form.title} onChange={e => set("title", e.target.value)} placeholder="Copy the listing title here…" className="w-full bg-[#1a2030] border border-[#263147] rounded-lg px-3 py-2 text-[13px] text-[#e8ecf4] outline-none focus:border-emerald-500/60 placeholder:text-[#546380]" />
            </div>
          </div>
