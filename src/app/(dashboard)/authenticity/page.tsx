    ...
    <AppShell title="Authenticity Intelligence" subtitle="Paste a Vinted link — we fetch the real listing and score it live">
      <div className="max-w-2xl">
        ...
        <div className="bg-[#141820] border border-[#1e2535] rounded-xl p-5 mb-4">
          <label className="text-[11px] font-semibold text-[#e8ecf4] block mb-2">Paste a Vinted listing URL</label>
          <div className="flex gap-2">
            <input value={url} onChange={e => setUrl(e.target.value)} onKeyDown={e => e.key === "Enter" && analyzeUrl()}
              placeholder="https://www.vinted.es/items/..." className="flex-1 bg-[#1a2030] border border-[#263147] rounded-lg px-3 py-2.5 font-mono text-[13px] text-[#e8ecf4] outline-none focus:border-blue-500 placeholder:text-[#546380]" />
            <button onClick={analyzeUrl} disabled={loading || !url.trim()} className="bg-emerald-400 text-[#0B0D10] font-mono font-bold text-[12px] px-5 rounded-lg hover:bg-emerald-300 transition-colors disabled:opacity-50 whitespace-nowrap">
              {loading ? "…" : "CHECK →"}
            </button>
          </div>
          {result?.fetched_listing && (
            <div className="mt-3 text-[11px] text-[#8fa3c4] bg-[#1a2030] rounded-lg px-3 py-2 flex items-center gap-1.5">
          ...
          <button onClick={() => setShowManual(s => !s)} className="mt-3 text-[11px] text-[#546380] hover:text-[#8fa3c4]">
            {showManual ? "− Hide manual entry" : "+ Or enter details manually"}
          </button>
        </div>

        {/* MANUAL MODE — collapsible fallback */}
        <div className="bg-[#141820] border border-[#1e2535] rounded-xl p-5 mb-4" style={{ display: showManual ? "block" : "none" }}>
          <div className="grid grid-cols-2 gap-3 mb-4">
            {[["Listed price (€)*","listed_price","45","number"],["Brand","brand","Gucci","text"],["Model","model","GG Belt","text"],["Category","category","Bags","text"],["Seller account age (days)","seller_days","365","number"],["Seller rating (0-5)","seller_rating","4.8","number"],["Seller total sales","seller_sales","42","number"]].map(([l,k,ph,t]) => (
              <div key={String(k)} className={k === "listed_price" ? "col-span-2" : ""}>
                <label className="text-[10px] text-[#546380] tracking-wide block mb-1.5">{l}</label>
                <input type={String(t)} value={form[k as keyof typeof form]} onChange={e => set(String(k), e.target.value)} placeholder={String(ph)} className="w-full bg-[#1a2030] border border-[#263147] rounded-lg px-3 py-2 font-mono text-[13px] text-[#e8ecf4] outline-none focus:border-blue-500 placeholder:text-[#546380]" />
              </div>
            ))}
            <div className="col-span-2">
              <label className="text-[10px] text-[#546380] tracking-wide block mb-1.5">Listing title (optional)</label>
              <input value={form.title} onChange={e => set("title", e.target.value)} placeholder="Copy the listing title here…" className="w-full bg-[#1a2030] border border-[#263147] rounded-lg px-3 py-2 font-mono text-[13px] text-[#e8ecf4] outline-none focus:border-blue-500 placeholder:text-[#546380]" />
            </div>
          </div>
          <button onClick={analyze} disabled={loading} className="w-full bg-emerald-500/10 border border-emerald-500 text-emerald-400 font-mono font-bold text-[13px] tracking-wide py-3 rounded-lg hover:bg-emerald-400 hover:text-[#0B0D10] transition-colors disabled:opacity-50">
          ...
          <>
            <div className="bg-[#141820] border border-[#1e2535] rounded-xl p-5 mb-4">
              <div className={`w-32 h-32 rounded-full border-4 ${styles.ring} ${styles.bg} flex flex-col items-center justify-center mx-auto mb-4`}>
                ...
                <div className={`text-[10px] font-bold tracking-wide ${styles.text}`}>{result.confidence_label}</div>
              </div>
              {result.flags.length > 0 ? (
                <div className="space-y-2">
                  {result.flags.map((f, i) => <div key={i} className="flex gap-2 bg-[#1a2030] rounded-lg px-3 py-2 text-[12px] text-[#8fa3c4]"><span></span><span>{f}</span></div>)}
                </div>
              ) : <div className="text-center text-[12px] text-[#546380]">No specific concerns detected</div>}
              <div className="mt-4 bg-amber-500/8 border border-amber-500/25 rounded-lg p-3 text-[11px] text-amber-400">{result.disclaimer}</div>
            </div>
            <div className="bg-[#141820] border border-[#1e2535] rounded-xl p-5">
              ...
              <div className="space-y-2">
                {Object.entries(result.breakdown).map(([k, v]) => (
                  <div key={k} className="flex justify-between py-2 border-b border-[#1e2535] last:border-0 text-[12px]">
