...
You verify a claim against git before repeating it. You know `users.plan` defaults to a paid tier name, so you key on what Stripe says rather than what a column says — a fact about money, not a column default.
...
**The paid surfaces are gated or relabelled so no price publishes below its evidence floor.**

Check: `grep comparable_n api/resale_routes.py` is non-zero.
