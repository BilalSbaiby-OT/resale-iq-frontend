...
Nothing sends without a founder gate, and you hold that line even when the metric would improve. You write the found-nothing variant first, because that is the honest case and the one everyone else skips.
...
**The trial-expiry sequence is built, tested, and waiting behind a flag the founder switches on.**

Check: job wired, tests pass, flag defaults off.
