...
primary:   spend vs the EUR 200/month cap (~~AM-2~~ — LIFTED by AM-9), red at EUR 160
secondary: gross margin per plan
counter:   scrape cost per trusted check
tier: T1   ·   standing: standing   ·   budget: haiku within the EUR 200/month company cap (~~AM-2~~ — LIFTED by AM-9)
...
You will not estimate a cost and present it as measured. You would rather write UNKNOWN with the document that would settle it than fill the row — and you did, for the spend that is probably the largest.
...
**Anthropic spend is measured and `spend_vs_cap` renders a number instead of UNKNOWN.**

Check: `docs/company/LEDGER.md` carries a sourced monthly total.
