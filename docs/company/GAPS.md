...
| **C12** — the `0.70` does two jobs | `max_buy_price = avg × 0.95 × 0.70`. The 0.70 is BOTH the reseller's margin (policy) AND a silent correction for the asking-vs-realized price gap (measurement). Found independently by `tech-lead` and `data-eng` | Never decomposed, never validated. If the true offer-discount is 10%, our advertised 30% margin is 20% and every customer who trusts it makes less than we said. Same applies to `TOLERANCE`, `MIN_VERDICT_COMPARABLES`, `MAX_ENDED_FRACTION` — nothing states which constants are policy and which are measurement |
| **C13** — a scraper block degrades confidence SILENTLY | `api/routes.py:530-599`: the staleness demotion fires only for **HIGH**. MEDIUM and LOW carry no staleness check at all, and the note selection is `if/elif` so a stale row hits the `n < 30` branch first — **the staleness note is written but structurally unreachable for most of the board** | Vinted has already blocked the production IP once. On a full block, most of the board would answer as confidently on day 10 as on day 1, with no signal. `pipeline_lag_min` exists and nothing customer-facing reads it |
| **C14** — `sold_at` is stamped at LABELLING time | `app_meta.str_diagnostic_v1` diagnosed it and nobody surfaced it: when a model is newly added to the catalog, the backlog of matching listings gets `sold_at` set to when WE recognised it, not when it left the shelf | The **third** instance of derived-becoming-source. `momentum_label` and `opportunity_score` (`db/queries.py:2112`, `:2132`) are computed off `sold_7d` trend, so a newly-covered model shows a fake 'hot right now' spike that is a labelling artefact, not market movement |
| **C15** — `parse_item` has FIVE exits, not one | C8 recorded the brand-drop. There are also `price_val <= 0`, a post-normalisation brand check, a price-block return, and **`except Exception: logger.debug(...); return None`** | **That exception handler is the widest hole in the codebase.** Any error in ~130 lines of parsing silently removes the listing from the shelf pass, which `detect_ended` reads as a departure → `sold_observed = 1`. Logged at DEBUG, which production almost certainly does not emit — the failure is not merely silent, it is designed to be invisible. Related: `active_listings` undercounting **inflates** `str_pct`, the same defect inverted |
