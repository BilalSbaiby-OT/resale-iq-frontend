...
| **C6** — REWORK REQUIRED, **must not merge** | `af4042d` adds the five rates correctly (all five verified, no inverted rate) but its drop-the-row behaviour runs on the INGEST path too, where `engine/shelf.py:222` reads an absent row as *left the shelf* = a **sale** → `mark_listing_sold` → `sold_observed=1` | **Failing closed for FX fails OPEN for sale detection**, manufacturing fabricated sales into the very column C4 promotes to ground truth. Trigger: `.get("currency_code","EUR")` returns `None` on a JSON null, so EUR-market rows that used to convert at 1.0 now vanish. This is the 168,852-fabricated-sales failure of `shelf.py` v1 in a new dress |
