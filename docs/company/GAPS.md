...
| **C8** — NEW, found while proving C6 | `parse_item` still returns `None` when `brand_title` is empty (`12101c4`, measured 0.58% of items). That drop reaches `engine.shelf.detect_ended` by the identical path, so it **asserts a sale** the same way the C6 bug would have | Live on `main` today, ~0.58% of every pass. It was reasoned about as losing ~4 rows of DATA; nobody noticed it also claims a SALE. Needs its own measurement: how many `sold_observed=1` rows trace to a brandless drop? |
