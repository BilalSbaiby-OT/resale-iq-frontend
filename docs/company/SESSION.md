...
| `claude/content-social/sold-relabel` | Step 1 of `tech-lead`'s A16 ordering (APPROVALS.md): relabels every "sold" claim as "asking price at departure" / "watched departures", methodology names the relist-bias mechanism. Self-verification pass (2026-09-01, `f66273e`) found the original catalog missed the programmatic SEO estate — `/category`, `/flip` hub + brand + brand×category pages, `flip-narrative.ts`, `/manual` — plus one Spanish blog test page making the identical false claim in Spanish; all now closed. `i18n.ts` still has no de/it/pt (pre-existing, unchanged by this branch — flagged, not fixed, as out of scope for a copy-only PR). `sold_observed`/`sold_7d`/`sold_at` untouched. `docs/audit/proof/W36/sold-relabel/proof.sh` 26/26 cold. Verified this session: `npx tsc --noEmit` clean, eslint clean on touched files, all 5 `check:*` green. Did NOT run `npm run build` or e2e — not in this session's grant |
