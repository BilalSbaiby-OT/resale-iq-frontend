...
| **W13** | **Claude/API spend is UNKNOWN; blocks spend_vs_cap KPI scoring.** UNIT-ECONOMICS.md analysis shows EUR 200 cap will break at 30–50 customers if costs scale at €5/account. Without measured spend, company cannot report cap status. LEDGER.md names this as the blocker. | `finance-ops` | `finance-ops` | **OPEN** — export Anthropic console usage last 30 days, calculate monthly cost, report in LEDGER.md |
| **W14** | **Trial→paid rate is 5–15% from industry average, never measured in company.** Central scenario in PATH-TO-TEN requires 33k visitors for 100 customers; at 15% trial→paid (company never asked) vs 10% (pessimistic), gap narrows by 25% (22k vs 33k visitors). Trial machine is the single highest-ROI funnel lever. | `finance-ops` | `backend-eng` (build trial-expiry) + `founder` (approve ship) | **OPEN** — PATH-TO-TEN §2 item 8, code ready for founder approval |
| **W15** | **PENDING and LIMIT_REACHED blockers hide 24% of traffic from conversion.** 19 of 78 requests silently fail (`PENDING`); another set show "NO DATA" with no upgrade CTA (`LIMIT_REACHED`). These artificially depress funnel metrics. Both ≤2h fixes already scoped. | `finance-ops` | `backend-eng` | **OPEN** — PATH-TO-TEN §2 items 3–4, scoped ≤2h each |
| **W16** | **Organic CAC is estimated at €100–150 at current burn, breakeven with 6-month LTV. Paid CAC > €200 will not pay back.** Decision framework: stay organic for next 6 months, measure LTV and churn, switch to paid only when LTV > €500 or visitors > 500/mo. | `finance-ops` | `CEO` (channel decision) | **OPEN** — UNIT-ECONOMICS.md §5 lays the case for 6-month organic-only strategy |
| **W17** | **100 customers requires realistic trial→paid + funnel fixes, not 4–6 month organic timeline.** Central estimate: 33k visitors at current 150/mo = 220 months. Even at optimistic (15% trial→paid, 3.5% signup), still 89 months. Timeline depends on trial machine (2–3× lever), organic growth 2–3×, PENDING/LIMIT_REACHED fixes. | `finance-ops` | `CEO` (reset KPI expectations) | **OPEN** — UNIT-ECONOMICS.md §7 "The Honest Paragraph" documents why founder's 4–6 month estimate is unachievable without measured levers |
